...
public class ListPane extends JPanel implements ActionListener,
		SmaliEntryChanged, ListSelectionListener {
	...
	private static final String ACTION_F2_KEY = "theJListAction_F2";
	...
	private Action actionF2Listener;
	...
	public ListPane() {
		...
		actionF2Listener = new AbstractAction() {
			/**
			 * 
			 */
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				JList<CommonEntry> source = (JList<CommonEntry>) actionEvent
						.getSource();
				...
				if (selectedCommonEntry != null) {
					...
					while (s.equals("NULL")) {
						if (selectedCommonEntry.type == 1) { // head
							...
							s = (String) JOptionPane.showInputDialog(
									ListPane.this, "Rename class head: "
											+ he.classNameSelf, "Rename",
									JOptionPane.PLAIN_MESSAGE, null, null,
									he.classNameSelf);
						} else if (selectedCommonEntry.type == 2) { // field
							FieldEntry fe = (FieldEntry) selectedCommonEntry;
							s = (String) JOptionPane.showInputDialog(
									ListPane.this, "Rename class field"
											+ fe.classFieldName, "Rename",
									JOptionPane.PLAIN_MESSAGE, null, null,
									fe.classFieldName);
							...
							s = (String) JOptionPane
									.showInputDialog(
											ListPane.this,
											"Rename class method"
													+ me.classConstructorName != null ? me.classConstructorName
													: me.classMethodName,
											"Rename",
					...
					if (s != null) {
						if (selectedCommonEntry.type == 1) {// head, nothing to
															// do

						} else if (selectedCommonEntry.type == 2) { // field
							FieldEntry fe = (FieldEntry) selectedCommonEntry;
							SmaliLoader.getInstance().renameField(
									selectedCommonEntry.se, fe, s);
							listmodel.Refresh(selectIndex);
						} else if (selectedCommonEntry.type == 3) { // method
							MethodEntry me = (MethodEntry) selectedCommonEntry;
							SmaliLoader.getInstance().renameMethod(
									selectedCommonEntry.se, me, s);
							listmodel.Refresh(selectIndex);
						}
			...
			if (!lsm.isSelectionEmpty()) {
				...
				for (int i = minIndex; i <= maxIndex; i++) {
					if (lsm.isSelectedIndex(i)) {
						...
						System.out.println("id:" + item.id + ", offset:"
								+ item.offset);
